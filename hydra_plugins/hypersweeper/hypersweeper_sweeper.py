...
class HypersweeperSweeper:
    ...
    def run(self, verbose=False):
        ...
        # Check if the optimizer has a custom `optimize` method.
        # This allows a facade like CostAwareFacade to run its own loop.
        if isinstance(self.optimizer.smac, CostAwareFacade):
            log.info("Optimizer has a custom `optimize` method. Handing over control.")
            # We need to wrap the hydra task_function to match what SMAC expects
            def smac_target_function(config, seed=None):
                # Convert SMAC config to a list of override strings
                overrides = [f"{key}={value}" for key, value in config.items()]
                
                # Add the seed override if provided by SMAC
                if seed is not None:
                    overrides.append(f"{self.seed_keyword}={seed}")

                # Combine with global overrides
                job_overrides = tuple(self.global_overrides) + tuple(overrides)

                # Use the launcher to run a single job.
                results = self.launcher.launch([job_overrides], initial_job_idx=self.job_idx)
                self.job_idx += 1
                
                job_return = results[0]

                # --- FIX 1: Handle Job Failure Gracefully ---
                # Check for failure and provide a valid failure cost to SMAC
                if job_return.return_value is None:
                    log.warning(
                        f"Job failed or returned None. Status: {job_return.status}. Overrides: {job_return.overrides}"
                    )
                    # Return max cost and min reward to SMAC so it can continue.
                    return {"reward_mean": -float('inf'), "runtime": self.max_budget}

                return job_return.return_value

            # Replace the dummy target function in the facade with the real one
            self.optimizer.smac.target_function = smac_target_function

            # Now, call the facade's custom optimize method
            incumbent = self.optimizer.smac.optimize()
            log.info(f"Finished custom optimization. Incumbent: {incumbent}")
            log.info("SMAC optimization finished. Per-trial output files were generated by the launcher.")

            # Manually populate self.history from SMAC's runhistory to generate output files.
            runhistory = self.optimizer.smac.runhistory
            if not runhistory.empty():
                log.info("Populating history from SMAC runhistory to generate summary files...")
                configs, performances, budgets = [], [], []

                # Group results by config_id if seeds are used
                if self.seeds:
                    from collections import defaultdict
                    grouped_results = defaultdict(lambda: {"performances": [], "budgets": [], "seeds": []})
                    for trial_key, trial_value in runhistory.items():
                        config_id = trial_key.config_id
                        # SMAC minimizes, so cost is -reward if we are maximizing
                        performance = -trial_value.cost if self.maximize else trial_value.cost

                        grouped_results[config_id]["performances"].append(performance)
                        grouped_results[config_id]["budgets"].append(trial_value.time)
                        grouped_results[config_id]["seeds"].append(trial_key.seed)
                        if "config" not in grouped_results[config_id]:
                            grouped_results[config_id]["config"] = runhistory.get_config(config_id)

                    # Use the existing methods to populate self.history and write the files
                    for config_id, data in grouped_results.items():
                        self.history["config"].append(data["config"])
                        self.history["performance"].append(np.mean(data["performances"]))
                        self.history["budget"].append(np.mean(data["budgets"]))

                        # Create a map of seed to performance for the current config
                        perf_by_seed = dict(zip(data["seeds"], data["performances"]))

                        # Append performance for each seed, or NaN if not run
                        for seed in self.seeds:
                            perf = perf_by_seed.get(seed, np.nan)
                            self.history[f"performance_{self.seed_keyword}_{seed}"].append(perf)

                    self._write_csv(self.history, "runhistory")

                else:  # No seeds
                    for trial_key, trial_value in runhistory.items():
                        configs.append(runhistory.get_config(trial_key.config_id))
                        performance = -trial_value.cost if self.maximize else trial_value.cost
                        performances.append(performance)
                        budgets.append(trial_value.time)
                    self.write_history(performances, configs, budgets)

                self.write_incumbents()
                log.info("Successfully generated runhistory.csv and incumbent.csv")

                # Now that history is populated, we can get the final incumbent
                inc_config, inc_performance = self.get_incumbent()
                # The backend expects only the configuration object (or a list of them).
                return [inc_config]
            else:
                log.warning("Optimizer finished without completing any trials. Returning None.")
                return None

        # --- Fallback to the original ask-and-tell loop ---
